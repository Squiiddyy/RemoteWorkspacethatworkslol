...
import net.mcreator.minksandmisfits.MinksandmisfitsModPlayerAnimationAPI;
...
public abstract class PlayerAnimationMixin<T extends LivingEntity> {
	...
	public void setupPivot(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, CallbackInfo ci) {
		if (master == null)
			master = "minksandmisfits";
		if (!master.equals("minksandmisfits"))
			return;
		...
		if (entityIn instanceof Player player_)
			...
			return;
		MinksandmisfitsModPlayerAnimationAPI.PlayerAnimation animation = MinksandmisfitsModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null)
			return;
	...
	public void setupAnim(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, CallbackInfo ci) {
		if (ageInTicks <= 0)
			return;
		if (!master.equals("minksandmisfits")) {
			if (!MinksandmisfitsModPlayerAnimationAPI.animations.isEmpty())
				MinksandmisfitsModPlayerAnimationAPI.animations.clear();
			return;
		}
		PlayerModel<T> model = (PlayerModel<T>) (Object) this;
		Player player = null;
		...
		if (data.getBoolean("ResetPlayerAnimation")) {
			...
			MinksandmisfitsModPlayerAnimationAPI.active_animations.put(player, null);
		}
		...
		MinksandmisfitsModPlayerAnimationAPI.PlayerAnimation animation = MinksandmisfitsModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null) {
			animation = MinksandmisfitsModPlayerAnimationAPI.animations.get(playingAnimation);
			if (animation == null) {
				MinksandmisfitsMod.LOGGER.info("Attepted to play null animation " + playingAnimation + ", did animations fail to load?");
				return;
			}
			MinksandmisfitsModPlayerAnimationAPI.active_animations.put(player, animation);
		}
		...
		for (Map.Entry<String, MinksandmisfitsModPlayerAnimationAPI.PlayerBone> entry : animation.bones.entrySet()) {
			String boneName = entry.getKey();
			MinksandmisfitsModPlayerAnimationAPI.PlayerBone bone = entry.getValue();
			ModelPart modelPart = getModelPart(model, boneName);
			if (modelPart == null)
				continue;
			// Apply rotation
			Vec3 rotation = MinksandmisfitsModPlayerAnimationAPI.PlayerBone.interpolate(bone.rotations, animationProgress, player);
			if (rotation != null) {
			...
			Vec3 position = MinksandmisfitsModPlayerAnimationAPI.PlayerBone.interpolate(bone.positions, animationProgress, player);
			if (position != null) {
			...
			// Apply scale
			Vec3 scale = MinksandmisfitsModPlayerAnimationAPI.PlayerBone.interpolate(bone.scales, animationProgress, player);
			if (scale != null) {
