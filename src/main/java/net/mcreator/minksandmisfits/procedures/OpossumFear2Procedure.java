...
@EventBusSubscriber
public class OpossumFear2Procedure {
	@SubscribeEvent
			...
			if (!world.getEntitiesOfClass(OpossumEntity.class, new AABB(Vec3.ZERO, Vec3.ZERO).move(new Vec3(x, y, z)).inflate(10 / 2d), e -> true).isEmpty()) {
				if (entity instanceof Mob _entity)
					_entity.getNavigation().moveTo((entity.getX() + entity.getX() - (findEntityInWorldRange(world, OpossumEntity.class, x, y, z, 10)).getX()),
							(entity.getY() + entity.getY() - (findEntityInWorldRange(world, OpossumEntity.class, x, y, z, 10)).getY()), (entity.getZ() + entity.getZ() - (findEntityInWorldRange(world, OpossumEntity.class, x, y, z, 10)).getZ()), 1);
			}
